{
    let sliders = V(".v-slider-1");
    sliders.forEach(element => {
        let children = element.children(".v-slider-1-slids");
        ...
        for (let i = count; i > 0; i--) {
            ...
            element.children('.v-slider-1-switches').put(elem.array()[0], 'prepend');
        }
        V('.v-slider-1-switches label').event({
            events: ['click'],
            funcs: [
                function(){
                    if(!V(this).hasClass("v-active-slide")){
                        ...
                        V(this).parent('.v-slider-1').children(".v-slider-1-items").css({
    ...
    let duration = V('.v-slider-1 .v-slider-1-items').css('transition-duration');
    duration = parseFloat(duration);

    function rb(el){
        el.attr('disabled', true);
        let active = el.parent('.v-slider-1').children('.v-slider-1-switches label');
    ...
    V('.v-slider-1 .v-slider-1-rb').event({
        events: ['click'],
        funcs: [
            function(){
                return rb(V(this));
    ...
    V('.v-slider-1 .v-slider-1-lb').event({
        events: ['click'],
        funcs: [
            function(){
                return lb(V(this));
